import java.util.*;
public class SetList 
{
	private ArrayList<String> sets = new ArrayList<String>();
	
	public SetList()
	{
		sets.add("Phyrexia: All Will Be One");
		sets.add("The Brothers' War");
		sets.add("Dominaria: United");
		sets.add("Streets of New Capenna");
		sets.add("Kamigawa: Neon Dynasty");
		sets.add("Innistrad: Crimson Vow");
		sets.add("Innistrad: Midnight Hunt");
		sets.add("Adventures in the Forgotten Realms");
		sets.add("Modern Horizon 2");
		sets.add("Strixhaven");
		sets.add("Kaldheim");
		sets.add("Zendikar Rising");
		sets.add("Core Set 2021");
		sets.add("Ikoria: Lair of Behemoths");
		sets.add("Commander 2020");
		sets.add("Theros Beyond Death");
		sets.add("Throne of Eldraine");
		sets.add("Commander 2019");
		sets.add("Core Set 2020");
		sets.add("Modern Horizons");
		sets.add("War of the Spark");
		sets.add("Ravnica Allegiance");
		sets.add("Guilds of Ravnica");
		sets.add("Commander 2018");
		sets.add("Core Set 2019");
		sets.add("Battlebond");
		sets.add("Dominaria");
		sets.add("Rivals of Ixalan");
		sets.add("Ixalan");
		sets.add("Commander 2017");
		sets.add("Hour of Devastation");
		sets.add("Amonkhet");
		sets.add("Aether Revolt");
		sets.add("Commander 2016");
		sets.add("Conspiracy: Take the Crown");
		sets.add("Kaladesh");
		sets.add("Eldritch Moon");
		sets.add("Shadows Over Innistrad");
		sets.add("Oath of the Gatewatch");
		sets.add("Commander 2015");
		sets.add("Battle for Zendikar");
		sets.add("Magic Origins");
		sets.add("Dragons of Tarkir");
		sets.add("Fate Reforged");
		sets.add("Commander 2014");
		sets.add("Khans of Tarkir");
		sets.add("M15");
		sets.add("Conspiracy");
		sets.add("Journey into Nyx");
		sets.add("Born of the Gods");
		sets.add("Commander 2013");
		sets.add("Theros");
		sets.add("M14");
		sets.add("Dragon's Maze");
		sets.add("Gatecrash");
		sets.add("Return to Ravnica");
		sets.add("M13");
		sets.add("Avacyn Restored");
		sets.add("Dark Ascension");
		sets.add("Innistrad");
		sets.add("M12");
		sets.add("New Phyrexia");
		sets.add("Mirrodin Besieged");
		sets.add("Scars of Mirrodin");
		sets.add("M11");
		sets.add("Rise of the Eldrazi");
		sets.add("Worldwake");
		sets.add("Zendikar");
		sets.add("M10");
		sets.add("Alara Reborn");
		sets.add("Conflux");
		sets.add("Shards of Alara");
		sets.add("Eventide");
		sets.add("Shadowmoor");
		sets.add("Morningtide");
		sets.add("Lorwyn");
		sets.add("Tenth Edition");
		sets.add("Future Sight");
		sets.add("Planar Chaos");
		sets.add("Time Spiral");
		sets.add("Coldsnap");
		sets.add("Dissension");
		sets.add("Guildpact");
		sets.add("Ravnica: City of Guilds");
		sets.add("Ninth Edition");
		sets.add("Saviors of Kamigawa");
		sets.add("Betrayers of Kamigawa");
		sets.add("Champions of Kamigawa");
		sets.add("Fifth Dawn");
		sets.add("Darksteel");
		sets.add("Mirrodin");
		sets.add("Eighth Edition");
		sets.add("Scourge");
		sets.add("Legions");
		sets.add("Onslaught");
		sets.add("Judgment");
		sets.add("Torment");
		sets.add("Odyssey");
		sets.add("Seventh Edition");
		sets.add("Apocalypse");
		sets.add("Planeshift");
		sets.add("Invasion");
		sets.add("Prophecy");
		sets.add("Nemesis");
		sets.add("Mercedian Masques");
		sets.add("Sixth Edition");
		sets.add("Urza's Destiny");
		sets.add("Urza's Legacy");
		sets.add("Urza's Saga");
		sets.add("Exodus");
		sets.add("Stronghold");
		sets.add("Tempest");
		sets.add("Fifth Edition");
		sets.add("Weatherlight");
		sets.add("Visions");
		sets.add("Mirage");
		sets.add("Alliances");
		sets.add("Homelands");
		sets.add("Ice Age");
		sets.add("Fourth Edition");
		sets.add("Chronicles");
		sets.add("Revised Edition");
		sets.add("Fallen Empire");
		sets.add("The Dark");
		sets.add("Legends");
		sets.add("Antiquities");
		sets.add("Arabian Nights");
		sets.add("Unlimited Edition");
		sets.add("Beta");
		sets.add("Alpha");
	}
	
	public int getNumberOfSets()
	{
		return sets.size();
	}
	
	public ArrayList<String> getList()
	{
		return sets;
	}
}
